...
describe('Rebuild what the authenticator signed', () => {
  ...
  it('01 — The TypeScript test verifies the reconstructed digest against the…', () => {
    expect.fail('The TypeScript test verifies the reconstructed digest against the credential\'s public key and locates the challenge bytes inside the raw clientDataJSON');
  });

  // The Foundry test asserts a valid payload returns 32 bytes whose value is one
  it('02 — The Foundry test asserts a valid payload returns 32 bytes whose value…', () => {
    expect.fail('The Foundry test asserts a valid payload returns 32 bytes whose value is one');
  });
  ...
  it('03 — Flipping one byte of r makes the Foundry test assert a zero-length…', () => {
    expect.fail('Flipping one byte of r makes the Foundry test assert a zero-length return, and the assertion is on returndatasize rather than on call success');
  });
});
