...
/**
 * What size of thing this is. The grain is the whole point of the ladder, so the page says it
 * rather than making every exercise look like every other exercise.
 */
const GRAIN_LABEL: Record<string, { label: string; blurb: string }> = {
  block: {
    label: 'Practice',
    blurb: 'An exercise over the two or three lessons that share a mechanism.',
  },
  module: {
    label: 'Module capstone',
    blurb: 'The build that closes out a module. It exercises the module\u2019s whole concept set at once, which is what makes it different from the exercises before it.',
  },
  exit: {
    label: 'Exit project',
    blurb: 'One build for a whole track, graded against the plain-English capabilities the track declares. Days, not hours.',
  },
};
...
/**
 * The practice screen, shared by both depths a practice can be addressed at.
 *
 * Depth is grain: a block exercise and a module capstone live under their module, a track's exit
 * project under the track. Both routes render this — the screen is identical, only the address and
 * the ladder above it differ.
 */
export async function PracticeScreen({ id }: { id: string }) {
  const p = getPractice(id);
  if (!p) notFound();

  const grain = GRAIN_LABEL[p.grain ?? 'block'] ?? GRAIN_LABEL.block;
  const mod = getModule(p.moduleId);
  const track = mod ? getTrack(mod.trackId) : undefined;
  if (!mod || !track) notFound();
  const moduleHref = `/t/${track.id}/${mod.id}`;

  const crumbs: Crumb[] = crumbsFor({ practiceId: p.id });
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1500px] gap-8 px-6 py-6">
      <Keyboard
        up={moduleHref}
        prev={prev ? hrefForPractice(prev.id) ?? undefined : undefined}
        next={next ? hrefForPractice(next.id) ?? undefined : undefined}
      />
      ...
      <main className="min-w-0 flex-1">
        ...
        <p className="mt-2 max-w-[62ch] text-[13px] leading-relaxed text-[var(--color-ink-2)]">
          {grain.blurb}
        </p>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-[13px]">
          ...
          {/* An exit project spans the whole track, so naming its host module here would say it
              belongs to that module. It is the thing that comes after all of them. */}
          {track && (
          ...
          <section aria-labelledby="concepts" className="mt-8 max-w-[74ch]">
            ...
            <dl className="mt-3 flex flex-col gap-3">
              {concepts.map((c) => (
                <div key={c.id}>
                  <dt>
                    <Link href={hrefForConcept(c.id) ?? moduleHref} className="text-[14px] font-medium hover:text-[var(--color-accent)]">
                      {c.title}
                    </Link>
                  </dt>
        ...
        <nav aria-label="Practice navigation" className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-[var(--color-rule)] pt-4 text-[13px]">
          <span>
            {prev ? (
              <Link href={hrefForPractice(prev.id) ?? moduleHref} className="text-[var(--color-accent)] hover:underline">
                ← {prev.title}
              </Link>
            ) : (
          ...
          <span>
            {next ? (
              <Link href={hrefForPractice(next.id) ?? moduleHref} className="text-[var(--color-accent)] hover:underline">
                {next.title} →
              </Link>
            ) : (
            ...
            <section aria-labelledby="in-module">
              ...
              <ol className="mt-2 flex flex-col gap-1">
                {family.map((f) => (
                  <li key={f.id}>
                    <Link
                      href={hrefForPractice(f.id) ?? moduleHref}
