...
/**
 * A practice is prerendered under what it exercises. Depth is grain: every practice is
 * module-attached today, so every one sits here; a lesson- or track-grain practice gets its own
 * route at its own depth rather than a field distinguishing it.
 */
...
export default async function PracticePage({ params }: { params: Promise<Params> }) {
  const { practice: id, module: moduleParam } = await params;
  const p = getPractice(id);
  if (!p) notFound();
  // The URL must agree with the module this practice actually belongs to.
  if (p.moduleId !== moduleParam) notFound();
